...
6. 한라운드당 하나의 숫자를 입력하고 프로그램이 입력한 수와 <code>```COM```</code>의 해당 타일을 비교합니다.
7. 타일의 색이 같은 경우 숫자가 높은 쪽이 <code>```score```</code> 1점을 획득합니다.
8. 타일의 색이 다른 경우 숫자가 높은 쪽이 <code>```score```</code> 2점을 획득합니다.
9. 숫자 0은 숫자9를 이길 수 있습니다.
10. 당신은 한 게임에서의 <code>```score```</code> 차 만큼 <code>```Burrito```</code>를 획득하게 됩니다.
```